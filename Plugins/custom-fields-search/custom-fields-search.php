...
if ( ! function_exists( 'cstmfldssrch_add_to_admin_menu' ) ) {
	function cstmfldssrch_add_to_admin_menu() {
		bws_general_menu();
		$hook = add_submenu_page( 'bws_plugins', __( 'Custom Fields Search settings', 'custom-fields-search' ), 'Custom Fields Search', 'manage_options', "custom-fields-search.php", 'cstmfldssrch_page_of_settings' );
		add_action( "load-$hook", 'cstmfldssrch_screen_options' );
	}
}
							...
							<?php if ( is_plugin_active( 'custom-search-pro/custom-search-pro.php' ) || is_plugin_active( 'custom-search-plugin/custom-search-plugin.php' ) ) { ?>
								<td>
									...
									<?php foreach ( $meta_key_result as $value ) { ?>
										<label><input type="checkbox" <?php if ( in_array( $value, $cstmfldssrch_options['fields'] ) ) echo 'checked="checked"'; ?> name="cstmfldssrch_fields_array[]" value="<?php echo $value; ?>" /><span class="cstmfldssrch_value_of_metakey"><?php echo $value; ?></span></label><br />
									<?php }	?>
								</td>
							<?php } else {
								$i = 1; ?>
								<td>
									...
									if ( isset( $list_custom_key ) ) {
										...
										<?php } else { ?>
											<span class="bws_info"><?php _e( 'If the type of the post is not default - you need to install and activate the plugin', 'custom-fields-search' ); ?> <a href="http://bestwebsoft.com/products/custom-search/download">Custom Search</a>.<span>
										<?php } 
									} ?>
								</td>
							<?php }
...
/* Screen option */
if ( ! function_exists( 'cstmfldssrch_screen_options' ) ) {
